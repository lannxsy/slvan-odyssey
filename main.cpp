...
float dt=0, lastFrame=0, gameTime=0, hitCooldown=0;
...
const glm::vec3 COL_SKY(0.55f,0.78f,0.98f);
const glm::vec3 COL_SUN(1.0f,0.95f,0.35f);
const glm::vec3 COL_GROUND(0.32f,0.58f,0.24f);
const glm::vec3 COL_GROUND_TOP(0.42f,0.72f,0.32f);
const glm::vec3 COL_WALL(0.2f,0.75f,0.35f);
const glm::vec3 COL_WALL_EDGE(0.35f,0.95f,0.5f);
const glm::vec3 COL_PLAYER_BODY(0.2f,0.55f,0.95f);
const glm::vec3 COL_PLAYER_SKIN(0.96f,0.8f,0.62f);
const glm::vec3 COL_PLAYER_LIMB(0.12f,0.22f,0.6f);
const glm::vec3 COL_EYE_WHITE(0.95f,0.95f,0.95f);
const glm::vec3 COL_EYE_BLACK(0.05f,0.05f,0.05f);
const glm::vec3 COL_ROCKET_BODY(0.85f,0.85f,0.9f);
...
void resetGame(){
    ...
    gameState=PLAYING; playerLives=5;
...
void takeDamage(){
    ...
    if(playerLives<=0){ gameState=GAME_OVER; return; }
    ...
    while(!glfwWindowShouldClose(win)){
        ...
        if(gameState==PLAYING){
            ...
            if(ms>0&&ms>lastMilestone){
                ...
                for(int k=0;k<2;k++) rockets.push_back({xd(rng), player.pos.y+8.f, -(7.f+ms*.3f), 0, true});
            }
            for(auto& r:rockets){
                if(!r.alive) continue;
        ...
        drawCube(shaderProg,{0,3.5f+camOff,-9},{22,20,0.3f},COL_SKY);
        drawCube(shaderProg,{0,6.5f+camOff,-7},{0.6f,0.6f,0.2f},COL_SUN);
        drawCube(shaderProg,{0,-0.3f,0},{15,0.3f,2.5f},COL_GROUND);
        drawCube(shaderProg,{0,0.02f,0},{15,0.05f,2.5f},COL_GROUND_TOP);
        setUniforms(shaderProg,proj,view);
        for(auto& w:walls){
            if(!w.alive||w.waitingDelay) continue;
            glm::vec3 wc=w.isFrozen?COL_WALL*0.82f:COL_WALL;
            glm::vec3 edge=w.isFrozen?COL_WALL_EDGE*0.82f:COL_WALL_EDGE;
            drawCube(shaderProg,{w.x,w.y,0},{w.width,w.height,0.55f},wc);
            drawCube(shaderProg,{w.x,w.y+w.height,0},{w.width+0.02f,0.02f,0.58f},edge);
        }
        {
        ...
        for(auto& r:rockets){
            ...
            drawCube(shaderProg,{r.x,r.y,0},      {0.18f,0.75f,0.18f},COL_ROCKET_BODY);
            drawCube(shaderProg,{r.x,r.y-0.6f,0}, {0.12f,0.25f,0.12f},COL_ROCKET_NOSE);
        ...
        drawPlayer(shaderProg,player.pos,0.58f,leg,arm);
        {
            ...
            if(!rockets.empty()){
                ...
                renderText(w2,SCR_W/2.f-w2.size()*cw*.8f/2,ch+20,.8f*sc,1,.3f,.05f);
            }
            if(hitCooldown>0){
            ...
            if(gameState==GAME_OVER){
                float ox=SCR_W/2.f, oy=SCR_H/2.f;
                renderRect(0,0,(float)SCR_W,(float)SCR_H,0.05f,0.01f,0.01f,0.75f);
                renderRect(ox-180,oy-115,360,230,0.1f,0.02f,0.02f,0.97f);
